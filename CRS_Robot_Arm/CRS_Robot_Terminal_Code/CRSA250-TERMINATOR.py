...
# Create the User Interface    
    ...
    # Create GUI
        ...
        self.webcam_button = tk.Button(self.cam_buttons_frame, text="Webcam", command=self.start_webcam_thread, bg='red')
...
# Serial Connection
    ...
    def send_program(self, select_all=False):
        if select_all:
            ...
            self.program_box.tag_add("sel", "1.0", "end")
        else:
    ...
    def process_commands(self, commands):
        for command in commands:
            if "(" in command and ")" in command:
                ...
                comment_text = command[comment_start:comment_end+1].strip().upper()
                if comment_text in command_list:
                    self.process_commented_command(comment_text)
                else:
                    print(f"Command not found: {comment_text}")
            else:
                self.send_command_serial(command.strip())


# Create New Functions to control the robot and Software
    def process_commented_command(self, command):
        if command in command_list:
            command_info = command_list[command]
            ...
            if requires_webcam and not self.webcam_running:
                ...
                if 'A' in command_info.get('modes', ''):
                    ...
                    function = command_info['app']
                    function()
                elif requires_connection and not self.CRS_Connected:
                    print("CRS not Connected or Detected.")
                else:
                    # This is a robot command, send to serial port
                    self.send_command_serial(command)
        else:
            print(f"{command.strip()}")

    def send_command(self, event):
        # Define Variables
        text = self.program_box.get("1.0", "end").strip().upper()
        mdi_text = self.mdi_box.get().strip().upper()

        # Combine the text from the program_box and the mdi_box
        combined_text = f"{text}\n{mdi_text}".strip()

        # Process console commands for combined text
        console_commands = self.handle_commented_code(combined_text)
        self.process_commands(console_commands)
        ...
        self.program_box.tag_configure("comment", foreground="purple")
        
# Send Stuff to serial       
    def send_command_serial(self, command, is_app_command=False):
        if self.CRS_Connected:
            ...
            if is_app_command:
                print(f"App command: {command.strip()}")
            else:
                print("CRS not Connected or Detected.")
...
# Webcam Settings
    def show_webcam(self, camera_port=0, fps=30):
        ...
        try:
            camera_port = int(self.port_var.get())  # Get the value of port_var
        except ValueError:
            camera_port = 0  # Set default value if the input is not a valid integer
    ...
    def update_webcam(self):
        # pytesseract.pytesseract.tesseract_cmd = r'J:\Documents\CRS Robot Arm\CRS Robot Terminal Code\Tesseract-OCR\tesseract.exe'
        ...
        while self.camthread:
            ...
            if ret:
                ...
                # Run OCR when the flag is set
                if self.run_ocr:
                    gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
                    camText = pytesseract.image_to_string(gray)
                    print(camText)  # Print the recognized text to the console
                    self.run_ocr = False  # Reset the flag
            # Number recognition
                gray = cv2.cvtColor(frame, cv2.COLOR_RGB2GRAY)
                camText = pytesseract.image_to_string(gray)
                print(camText)  # Print the recognized text to the console
    ...
    def capture_image(self):
        if not self.webcam_running:
            self.status_bar.config(text="Webcam not connected!")
            return
        ...
        self.status_bar.config(text="Image saved to file")
